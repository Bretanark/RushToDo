...
`App` selects one of three routes from `window.location`: `/`, `/work-item/new` or `/work-item/{id}`. `LinkButton` retains a real `href` but uses the History API for ordinary in-app clicks; browser Back/Forward, refresh, Ctrl-click and opening in a new tab therefore retain normal web behaviour without a routing dependency.

`HomePage` loads incomplete WorkItems with `Api.searchWorkItems` and resolves their gardener names with `Api.getGardeners`. Each mobile-first tile is one `AppLink`; it shows values in edit-page order without repeated field labels, and the entire tile drills into the WorkItem.
...
Saving uses `Api.saveWorkItem`, which selects POST or PUT from `workItemId`, then returns home. The Back link turns red when the model differs from its loaded baseline. `Completed` and `Cancel Job` populate today’s corresponding business date, persist through the same API operation and return home.
...
`GardenerField` loads real lookup data through `Api.getGardeners()` and passes the resulting `LookupItem[]` to the generic `AutocompleteField`. During development, Vite proxies `/gardener` and `/work-item` to the API running at `https://localhost:63240`, so the browser sees the requests as same-origin and needs no CORS configuration.
